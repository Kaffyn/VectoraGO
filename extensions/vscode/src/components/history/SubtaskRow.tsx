...
const SubtaskRow = ({ node, depth, onToggleExpand, className }: SubtaskRowProps) => {
  ...
  return (
    <div data-testid={`subtask-row-${item.id}`} className={className}>
      ...
      {hasChildren && (
        <div className={cn("overflow-clip transition-all duration-300", isExpanded ? "max-h-[2000px]" : "max-h-0")}>
          {children.map((child) => (
            <SubtaskRow key={child.item.id} node={child} depth={depth + 1} onToggleExpand={onToggleExpand} />
          ))}
        </div>
      )}
    </div>
