...
const TaskGroupItem = ({
  ...
  return (
    <div
      ...
      className={cn("bg-vscode-editor-background rounded-xl border border-transparent overflow-hidden", className)}
    >
      {/* Parent task */}
      <TaskItem
        item={parent}
      ...
      {hasSubtasks && (
        <div
          ...
          className={cn("overflow-clip transition-all duration-500", isExpanded ? "max-h-[2000px] pb-2" : "max-h-0")}
        >
          {subtasks.map((node) => (
