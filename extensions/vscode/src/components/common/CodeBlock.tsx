...
const CodeBlock = memo(
  ({
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        const isAtBottom =
          Math.abs(preElement.scrollHeight - preElement.scrollTop - preElement.clientHeight) < SCROLL_SNAP_TOLERANCE;
    ...
    return (
      <CodeBlockContainer ref={codeBlockRef}>
        ...
        {!isSelecting && (
          ...
          >
            {showCollapseButton && (
              <StandardTooltip content={t(`chat:codeblock.tooltips.${windowShade ? "expand" : "collapse"}`)} side="top">
                <CodeBlockButton
                  onClick={() => {
