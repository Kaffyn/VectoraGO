    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="p-0 w-[var(--radix-popover-trigger-width)]">
        <Command shouldFilter={false}>
          ...
          <CommandList>
            <CommandEmpty>{searchValue && <div className="py-2 px-1 text-sm">{emptyMessage}</div>}</CommandEmpty>
            <CommandGroup>
              {filteredOptions.map((option) => (
                ...
                >
                  ...
                  <Check
                    className={cn("ml-auto h-4 w-4 p-0.5", value === option.value ? "opacity-100" : "opacity-0")}
                  />
                </CommandItem>
              ))}
            </CommandGroup>
