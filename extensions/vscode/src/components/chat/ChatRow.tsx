...
export const ChatRowContent = ({
  ...
  const [icon, title] = useMemo(() => {
    switch (type) {
      ...
      case "command":
        return [
          isCommandExecuting ? <ProgressIndicator /> : <TerminalSquare className="size-4" aria-label="Terminal icon" />,
          <span style={{ color: normalColor, fontWeight: "bold" }}>{t("chat:commandExecution.running")}</span>,
        ];
      case "use_mcp_server": {
      ...
      case "followup":
        return [
          <MessageCircleQuestion className="w-4 shrink-0" aria-label="Question icon" />,
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <span style={{ fontWeight: "500", fontSize: "var(--vscode-font-size)" }}>{skillInfo.skill}</span>
                  {skillInfo.source && (
              ...
              {isExpanded && (slashCommandInfo.args || slashCommandInfo.description) && (
                ...
                >
                  {slashCommandInfo.args && (
                    <div>
                      <span style={{ fontWeight: "500" }}>Arguments: </span>
                      <span style={{ color: "var(--vscode-descriptionForeground)" }}>{slashCommandInfo.args}</span>
                    </div>
                  )}
                  {slashCommandInfo.description && (
                    <div style={{ color: "var(--vscode-descriptionForeground)" }}>{slashCommandInfo.description}</div>
                  )}
                </div>
              )}
            </div>
              ...
              <div className="pl-6">
                <ToolUseBlock>
                  <div className="p-2">
                    ...
                    <div className="flex items-center gap-1 text-xs text-vscode-descriptionForeground">{tool.path}</div>
                  </div>
                </ToolUseBlock>
              </div>
            )}
  ...
  switch (message.type) {
    case "say":
      switch (message.say) {
        case "diff_error":
          return <ErrorRow type="diff_error" message={message.text || ""} expandable={true} showCopyButton={true} />;
        case "subtask_result":
        ...
        case "reasoning":
          return (
            <ReasoningBlock content={message.text || ""} ts={message.ts} isStreaming={isStreaming} isLast={isLast} />
          );
        case "api_req_started":
          ...
          switch (sayTool.tool) {
            ...
            case "readCommandOutput": {
              ...
              if (isSearch) {
                ...
                infoText = `${formatBytes(sayTool.readStart)} - ${formatBytes(sayTool.readEnd)} of ${formatBytes(
                  sayTool.totalBytes,
                )}`;
    ...
    case "ask":
      switch (message.ask) {
        ...
        case "command":
          return <CommandExecution executionId={message.ts.toString()} text={message.text} icon={icon} title={title} />;
        case "use_mcp_server":
