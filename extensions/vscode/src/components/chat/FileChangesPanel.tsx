      ...
      <CollapsibleContent>
        <div className="flex flex-col gap-1 pb-2 pl-6">
          {Array.from(byPath.entries()).map(([path, entries]) => {
            ...
            const hasMergedDiff = originalContent !== undefined && finalContent != null && finalContent !== "";
            ...
            return (
              <div key={path} className="rounded border border-vscode-panel-border overflow-hidden">
                <CodeAccordion
                  ...
                  diffStats={combinedStats.added > 0 || combinedStats.removed > 0 ? combinedStats : undefined}
                  onJumpToFile={
                    path
                      ? () =>
                          vscode.postMessage({
