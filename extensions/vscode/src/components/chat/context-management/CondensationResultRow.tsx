    ...
    <div className="mb-2">
      ...
      >
        <div className="flex items-center gap-2 flex-grow">
          ...
          <span className="font-bold text-vscode-foreground">{t("chat:contextManagement.condensation.title")}</span>
          <span className="text-vscode-descriptionForeground text-sm">
            {prevTokens.toLocaleString()} → {newTokens.toLocaleString()} {t("chat:contextManagement.tokens")}
          </span>
          <VSCodeBadge className={displayCost > 0 ? "opacity-100" : "opacity-0"}>${displayCost.toFixed(2)}</VSCodeBadge>
        </div>
